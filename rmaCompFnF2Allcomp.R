...
rmaNonparametric <- function(currentDate = currentDate, dataM, alpha = 0.05, minNi = 5, uniqueProduct = uniqueProduct, nList = nList, x_mid = x_mid, x = x, endMonth = endMonth){
  dataComp_c <- dataM[[2]]
...
rmaNonparametricC <- function(currentDate = currentDate, dataM, alpha = 0.05, minNi = 5, uniqueProduct = uniqueProduct, nList = nList, x_mid = x_mid, x = x, endMonth = endMonth){
  dataComp_c <- dataM[[2]]
  dat_censored1 <- dataM[[4]]
  # ----- Make the table, 1st column is time point, 2nd column is attribute. 
  # ----- 1. attribute = 1 means failure data
...
pro = 1700 # ok
pro = 1750 # no
pro = 1805 # have problems, time consuming
pro = 1754 # small data
pro = 1748 # have problems (resolved)
pro = 1766 # ok
pro = 1833 # strange!! problems (resolved)
pro = 1820 # time consuming (resolved)
pro = 1799 # have problems (resolved)
pro = 1771 # small data
pro = 114  # 
pro = 1996 # small data
pro = 1464 # soso
...
# portion of shortage
sum(cumulatedEmp < 0)/length(cumulatedEmp)
sum(cumulatedNon < 0)/length(cumulatedNon)
sum(cumulatedMVTrend < 0)/length(cumulatedMVTrend)
sum(cumulatedM < 0)/length(cumulatedM)
sum(cumulatedTs < 0)/length(cumulatedTs)
